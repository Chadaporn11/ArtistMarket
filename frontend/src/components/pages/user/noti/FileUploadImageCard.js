...
const FileUploadImageCard = ({ values, setValues, loading, setLoading }) => {
        ...
        if (files) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                Resize.imageFileResizer(
                    ...
                    0,
                    (uri) => {
                        axios.post(process.env.REACT_APP_VERCEL_API_URL+"/api/images-user",
                            {
                                image: uri,
                            },
                            {
    ...
    const handleRemove = (public_id) => {
        ...
        //#2
        const { images } = values;
        axios.post(process.env.REACT_APP_VERCEL_API_URL+"/api/removeimages-user",
            {
                public_id
            }, {
            headers: {
...
export default FileUploadImageCard